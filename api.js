class REST_API {
	 ...
	 */
	static async Request( url, endpoint, method = "GET", headers = {}, queryParams = null, body = null, data = null, onSuccess = () =>{}, onError = () => {}){
        ...
        const options = {
            ...
            error: (jqXHR, textStatus, errorThrown) => {
                console.error(`Error during API request to ${requestUrl}:`, textStatus, errorThrown);
                onError(errorThrown);
            }
        };
        ...
        return new Promise((resolve, reject) => {
            $.ajax(options)
                ...
                .fail((jqXHR, textStatus, errorThrown) => reject(new Error(`API request failed: ${textStatus} ${errorThrown.toString()}`)));
        });
    ...
    async Request( endpoint, method = "GET", headers = {}, queryParams = null, body = null, data = null, onSuccess = () =>{}, onError = () => {}){
        return await REST_API.Request(this.url, endpoint, method, headers, queryParams, body, data, onSuccess, onError);
    }

	async GetJson_Request(endpoint, method = "GET", headers = {}, queryParams = null, body = null, onSuccess = () =>{}, onError = () => {}) {
		return  await this.Request(endpoint, method, headers, queryParams, body, onSuccess, onError);
	}
}
...
class E6Api_ {
	...
	async GetPostInfo(md5, username, api) {
		...
		try{
			let json = await this.Request("/posts.json", "GET", username, api, null, {limit: 1, tags: 'md5:'+ md5});
			return json;
		} catch (error) {
	...
	async SetPostFavourite(api, username, postId) {
		...
		try {
			await this.Request('/favorites.json', "post", username, api, {}, {post_id: postId});
		}
		catch (error) {
	...
	async Request(endpoint, method = "GET", username, api, body = null, params = {}) {
		console.log(`e6_ApiRequest`);

		if (!username?.trim() || !api?.trim()) {
			console.error("e6_ApiRequest was called but username or api key was empty, skipping fetch");
			return null;
		}

		const headers = this.getHeaders(username, api);
		let  response = await this.REST_API.GetJson_Request(endpoint, method, headers, params, body)
		return response;
	}
}
...
class WalltakerApi_ {
	...
	async GetUserInfo(username, apiKey) {
		...
		try {
			return (await this.Request(`/users/${username}.json`, "GET", { api_key: apiKey}));
		} catch (error) {
	...
	async Request(endpoint, method = "GET", queryParams = {}, body = null, data = null, onSuccess = () => {}, onError = () =>{}) {
		return await this.REST_API.Request(endpoint, method, this.clientHeaders, queryParams, body, data, onSuccess, onError);
	}

	async PostReaction(linkID,reactType, reactText, apiKey, onSuccess = () => {}, onError = () => {}) {
		if(!WalltakerApi_.IsAPIKeyValid(apiKey)){
			console.error("Can't post reaction API-key is invalid!");
			return;
		}
		console.log('Posting reaction (' + reactType + ',"' + reactText + '") to Link ' + linkID);
		...
		await this.Request(`/links/${linkID}/response.json`, "POST", null,null,data, onSuccess,onError);
	}

	async GetLinkInfo(linkID,onError = () =>{}) {
		return (await this.Request(`/links/${linkID}.json`, "Get",{},null,null, ()=>{},onError));
	}
